...
function SignupInner() {
  ...
  async function handleEmailSignUp(e: React.FormEvent) {
    ...
    if (err) { setError(err.message ?? 'Could not create account. Please try again.'); return }
    ...
    <div className="fixed inset-0 z-[60] flex">
      ...
      <div className="w-full md:w-[540px] flex-shrink-0 bg-white flex flex-col px-14 py-10 overflow-y-auto">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-[340px] w-full mx-auto">
          <h1 className="font-display text-2xl font-normal text-black mb-1">Create your account</h1>
          ...
          {/* Sign up form */}
          <form onSubmit={handleEmailSignUp} className="flex flex-col gap-0">
            <label className="font-mono text-xs text-black mb-1.5 block">Name</label>
            <input
              type="text"
              value={name}
            ...
            <label className="font-mono text-xs text-black mt-4 mb-1.5 block">Email</label>
            <input
              type="email"
              value={email}
            ...
            {password.length > 0 && (
              <div className="mt-2 flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  {minLength
                    ? <CheckCircle size={14} className="text-[#3ECF8E] flex-shrink-0" />
                    : <Circle size={14} className="text-gray-400 flex-shrink-0" />}
                  <span className="font-mono text-xs text-gray-400">Must be at least 8 characters</span>
                </div>
                <div className="flex items-center gap-2">
                  {hasSpecial
                    ? <CheckCircle size={14} className="text-[#3ECF8E] flex-shrink-0" />
                    : <Circle size={14} className="text-gray-400 flex-shrink-0" />}
                  <span className="font-mono text-xs text-gray-400">Must contain one special character</span>
                </div>
              </div>
            )}
